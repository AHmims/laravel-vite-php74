...
it('generates URLs relative to the configured ASSET_URL in production', function () {
    ...
    expect(using_manifest('builds/public/with-css/manifest.json')->getTags())
        ->toContain(
            '<link rel="stylesheet" href="https://s3.us-west-2.amazonaws.com/12345678/with-css/assets/test.65bd481b.css" />'
        )
        ->toContain(
            '<script type="module" src="https://s3.us-west-2.amazonaws.com/12345678/with-css/assets/test.a2c636dd.js"></script>'
        );
});
...
it('returns a valid asset URL in development', function () {
    ...
    expect(vite()->getAssetUrl('without-leading-slash.txt'))->toContain(
        'http://localhost:3000/without-leading-slash.txt'
    );
});
...
it('returns a valid asset URL in production', function () {
    ...
    expect(vite()->getAssetUrl('/my-custom-asset.txt'))->toContain(
        'http://localhost/with/leading/slash/my-custom-asset.txt'
    );

    set_vite_config('default', ['build_path' => 'with/trailing/slash/']);
    expect(vite()->getAssetUrl('/my-custom-asset.txt'))->toContain(
        'http://localhost/with/trailing/slash/my-custom-asset.txt'
    );
